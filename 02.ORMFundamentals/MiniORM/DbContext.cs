    ...
    public abstract class DbContext
    {
        ...
        public void SaveChanges()
        {
            var dbSets = this.dbSetProperties
                .Select(propInfo => propInfo.Value.GetValue(this))
                .ToArray();
            ...
            where TEntity : class, new()
        {
            ...
            var collections = entityType
                .GetProperties()
                .Where(propInfo => propInfo.PropertyType.IsGenericType && propInfo.PropertyType.GetGenericTypeDefinition() == typeof(ICollection<>))
                .ToArray();
            ...
            where TDbSet : class, new() where TCollection : class, new()
        {
            ...
            var foreignKey = entityType.GetProperties()
                .First(propInfo => propInfo.HasAttribute<KeyAttribute>());
            ...
            if (isManyToMany)
            {
                primaryKey = collectionType.GetProperties()
                    .First(propInfo => collectionType.GetProperty(propInfo.GetCustomAttribute<ForeignKeyAttribute>().Name)
            ...
            where TEntity : class, new()
        {
            ...
            var foreignKeys = entityType.GetProperties()
                .Where(propInfo => propInfo.HasAttribute<ForeignKeyAttribute>())
                .ToArray();
            ...
            foreach (var foreignKey in foreignKeys)
            {
                ...
                var navigationPrimaryKey = navigationProperty.PropertyType.GetProperties().First(propInfo => propInfo.HasAttribute<KeyAttribute>());

                foreach (var entity in dbSet)
                {
                    var foreignKeyValue = foreignKey.GetValue(entity);

                    var navigationPropertyValue = ((IEnumerable<object>)navigationDbSet).First(currentNavigationProp => navigationPrimaryKey.GetValue(currentNavigationProp).Equals(foreignKeyValue));
